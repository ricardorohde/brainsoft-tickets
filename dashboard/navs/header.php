<?php 
  $sql = $connection->getConnection()->prepare("SELECT registry.name FROM client, registry WHERE client.id_credential = ? AND client.id_registry = registry.id");
  $sql->execute(array($id));

  $registry = $sql->fetchAll();

  $sqlNotify = $connection->getConnection()->prepare("SELECT notification.description, date_format(notification.date, '%d/%m/%Y') as nDate FROM notification, user_notification WHERE notification.id = user_notification.id_notification AND user_notification.id_user = ?");
  $sqlNotify->execute(array($id));

  $notifications = $sqlNotify->fetchAll();
          ...
          <div class="container-fluid">
            <div class="navbar-holder d-flex align-items-center justify-content-between">
              <div class="navbar-header"><a id="toggle-btn" href="#" class="menu-btn"><i class="fa fa-bars"> </i></a><a href="index.php" class="navbar-brand">
                  ...
                  <?php } else{ ?>
                    <div class="brand-text d-none d-md-inline-block"><span>Cartório </span><strong class="text-primary"> <?php echo $registry[0]['name']; ?></strong></div></a></div>
                  <?php } ?>
