...
<html lang="pt-br">
    ...
    <div class="root-page forms-page">
    	...
    	<?php
	        ...
	        $sql_chats = $connection->getConnection()->prepare("SELECT id_module, t_group, id_chat, id_attendant as id, registered_at FROM ticket WHERE t_status != ? AND t_status != ? AND t_status != ? ORDER BY id_chat desc");
	        ...
	        /* GRUPO 1 */
	        $sql_initialize_queue_1 = $connection->getConnection()->prepare("SELECT id_attendant FROM ticket WHERE t_status = :status AND t_group = :group ORDER BY registered_at DESC LIMIT :cont");
			...
			$sql_id_finalized_queue_1 = $connection->getConnection()->prepare("SELECT DISTINCT id_attendant FROM ticket, employee WHERE 
				((t_status = :status1 OR t_status = :status2) OR t_status = :status3) AND ticket.t_group = :group AND employee.on_chat = :active AND ticket.id_attendant = employee.id ORDER BY registered_at ASC LIMIT :cont");
	        $sql_id_finalized_queue_1->bindValue(':status1', "fechado", PDO::PARAM_STR);
			...
			/* GRUPO 2 */
			$sql_initialize_queue_2 = $connection->getConnection()->prepare("SELECT id_attendant FROM ticket WHERE t_status = :status AND t_group = :group ORDER BY registered_at DESC LIMIT :cont");
			...
			$sql_finalized_queue_2 = $connection->getConnection()->prepare("SELECT DISTINCT id_attendant FROM ticket, employee WHERE 
				((t_status = :status1 OR t_status = :status2) OR t_status = :status3) AND ticket.t_group = :group AND employee.on_chat = :active AND ticket.id_attendant = employee.id ORDER BY registered_at ASC LIMIT :cont");
			$sql_finalized_queue_2->bindValue(':status1', "fechado", PDO::PARAM_STR);
	        $sql_finalized_queue_2->bindValue(':status2', "solucionado", PDO::PARAM_STR);
	        $sql_finalized_queue_2->bindValue(':status3', "pendente", PDO::PARAM_STR);
        ...
        <div class="container-fluid">
          ...
          <div id="conteudo">
      		...
      		<div class="row" id="internal-row">
      			...
      			<?php $finalized_queue_group_1 = array(); $new_queue_group_1 = array();?>
      			<?php 
      				$fila = array();
				...
				<?php foreach($contagem as $numero => $vezes) {
					if($vezes == 2) {
						array_unshift($new_queue_group_1, $numero);
					} 
				} ?>
				...
				<?php 
					...
					foreach ($row_id_finalized_queue_1 as $id_attendant){
						...
						$date = strtotime($row_finalized_queue_1['registered_at']);
							            	...
							            	<?php foreach ($chats as $chat) : ?>
							            		<?php if ($chat['t_group'] == "nivel1") : ?>
							            			<?php 
							            				...
							            				$sql_chat_number->execute(array($chat['id_chat'])); $chat_number = $sql_chat_number->fetchAll(); 

							            				$sql_ticket_time = $connection->getConnection()->prepare("SELECT registered_at FROM ticket WHERE id_chat = ?"); 
							            				$sql_ticket_time->execute(array($chat['id_chat'])); $ticket_time = $sql_ticket_time->fetchAll();
							            			?>
							            			...
							            			<?php if ($chat['id'] == $new_queue) : ?>
							            				<div>
								            				...
								            				<button class="btn btn-secondary filha" data-container="body" data-toggle="popover" data-placement="bottom" data-html="true" data-content="
								            				...
								            				"><?php echo $chat_number[0]['id_chat'];?></button>

									            			<a href="#"></a>
									            			<input type="hidden" name="startedTime<?php echo $rand?>" value="<?php $chat_number[0]['opening_time']?>">

								            				<?php $test = $connection->getConnection()->prepare("SELECT limit_time FROM ticket_module WHERE id = ?"); $test->execute(array($chat['id_module'])); $testt = $test->fetchAll(); ?>
			...
			<div class="row" id="internal-row">
      			...
      			<?php $finalized_queue_group_2 = array(); $new_queue_group_2 = array();?>
      			<?php 
      				$fila = array();
				...
				<?php foreach($contagem as $numero => $vezes) {
					if($vezes == 2) {
						array_unshift($new_queue_group_2, $numero);
					} 
				} ?>
				...
				<?php 
					...
					foreach ($row_id_finalized_queue_2 as $id_attendant) {
						$sql_finalized_queue_2 = $connection->getConnection()->prepare("SELECT id_attendant, registered_at FROM ticket WHERE id_attendant = ? ORDER BY id DESC LIMIT 1");
						$sql_finalized_queue_2->execute(array($id_attendant['id_attendant'])); $row_finalized_queue_2 = $sql_finalized_queue_2->fetch();

						$date = strtotime($row_finalized_queue_2['registered_at']);
						$final_date = date('Y-m-d H:i:s', $date);
				...
				?>
				<?php $queue_merge_2 = array_merge($finalized_queue_group_2, $new_queue_group_2) ?>
										...
										<div class="card-body nivel2">
							            	<?php foreach ($chats as $chat) : ?>
							            		<?php if ($chat['t_group'] == "nivel2") : ?>
							            			<?php $sql_chat_number = $connection->getConnection()->prepare("SELECT id_chat, opening_time FROM chat WHERE id = ?"); $sql_chat_number->execute(array($chat['id_chat'])); $chat_number = $sql_chat_number->fetchAll(); ?>
							            			...
							            			<?php if ($chat['id'] == $new_queue) : ?>
							            				...
							            				<div>
								            				...
								            				<button class="btn btn-secondary filha" data-container="body" data-toggle="popover" data-placement="bottom" data-html="true" data-content="
								            					<div id='popover_content_wrapper'>
															   	<p><strong>Ticket: </strong><?= $chat_number[0]['id_chat'];?></p>
															   	<p><strong>Inicio: </strong><?= date('d/m/Y H:i:s', strtotime($chat_number[0]['opening_time']));?></p>
															   	<button id='btn-modal' class='btn btn-primary' value='<?= $chat_number[0]['id_chat'];?>' onClick='redirectToTicket(this.value)'>Visualizar Ticket</button>
																</div>
								            				"><?php echo $chat_number[0]['id_chat'];?></button>
			...
			<?php
				$sql_attendants = $connection->getConnection()->prepare("SELECT id, name, on_chat FROM employee WHERE t_group = ? OR t_group = ? ORDER BY t_group, name");
        		$sql_attendants->execute(array("nivel1", "nivel2")); $attendants = $sql_attendants->fetchAll();
			?>
